 ...
 *
 * Эта функция устанавливает формат отображения даты, включает всплывающее календарное окно,
 * устанавливает русскую локаль для календаря и устанавливает текущую дату.
 *
 * @param dateEdit Указатель на виджет редактирования даты (QDateEdit).
 */
void SetupDateEdit(QDateEdit *dateEdit) {
  dateEdit->setDisplayFormat("dd.MM.yyyy"); // Устанавливаем формат отображения даты на "день.месяц.год"
  dateEdit->setCalendarPopup(true); // Включаем всплывающее календарное окно
  dateEdit->calendarWidget()->setLocale(QLocale("ru_RU")); // Устанавливаем русскую локаль для календаря
  QDate date = QDate::currentDate(); // Получаем текущую дату
  dateEdit->setDate(date); // Устанавливаем текущую дату в виджете редактирования даты
}

/**
 * @brief Находит родительский макет, содержащий указанный виджет.
 *
 * Эта функция рекурсивно обходит дочерние элементы родительского макета и его дочерних макетов,
 * чтобы найти макет, содержащий указанный виджет.
 *
 ...
 * @return Указатель на найденный родительский макет, либо nullptr, если макет не найден.
 */
QLayout *FindParentLayout(QWidget *widget, QLayout *parent) {
  ...
  for (QObject *obj : parent->children()) {
    ...
    if (layout != nullptr) {
      ...
      // Если макет содержит другие дочерние элементы, вызываем функцию рекурсивно
 ...
 * @brief Находит родительский макет, содержащий указанный виджет.
 *
 * Эта функция рекурсивно обходит родительскую иерархию виджетов, чтобы найти макет, содержащий указанный виджет.
 *
 * @param widget Указатель на виджет, для которого ищется родительский макет.
 * @return Указатель на найденный родительский макет, либо nullptr, если макет не найден.
 */
QLayout *FindParentLayout(QWidget *widget) {
  ...
  if (widget->parentWidget() != nullptr) {
    ...
    if (widget->parentWidget()->layout() != nullptr) {
      // Вызываем вспомогательную функцию для поиска макета в родительском макете
...
/**
 * @brief Преобразует число типа double в строку типа QString с указанной точностью.
 *
 * Этот метод принимает число типа double и преобразует его в строку типа QString с указанной точностью.
 * Удаляются лишние нули в конце числа.
 *
...
QString DoubleToQStr(double num, double precision) {
  return QString::fromStdString(mathHelper::RemoveTrailingZero(QString::number(num, 'f', precision).toStdString()));
}
